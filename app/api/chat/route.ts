...
import { SendMessage } from '@/firebase/chats/firestore-action';
...
export async function POST(req: Request) {
  ...
  // Reuse the room provided by the client when it exists; otherwise start a new room.
  // Create the room up-front (before streaming) so the X-Room-Id response header
  // is correct and the client can reuse the same room on the next message.
  let room = roomId;
  if (!room) {
    room = nanoid();
